 ...
 *
 * Usage:
 *   node tools/generate-og-preview.mjs           # Preview local build
 *   node tools/generate-og-preview.mjs --local   # Preview local dev server
 *   node tools/generate-og-preview.mjs --prod    # Preview production site
 */
...
const mode = args.includes('--prod') ? 'prod' : args.includes('--local') ? 'local' : 'build';

// Load site configuration
const configPath = join(__dirname, '../src/site.config.ts');
const configContent = readFileSync(configPath, 'utf-8');

// Extract config values using regex (simple parser for .ts files)
function extractValue(key) {
  const match = configContent.match(new RegExp(`${key}:\\s*['"]([^'"]+)['"]`));
  return match ? match[1] : '';
}

const siteName = extractValue('name');
const siteUrl = extractValue('url');
const siteDescription = extractValue('description');
const twitterHandle = extractValue('twitterHandle');
...
<body>
  ...
  <div class="footer">
    <p>Generated by <a href="./generate-og-preview.mjs">tools/generate-og-preview.mjs</a></p>
    <p style="margin-top: 0.5rem;">Usage:</p>
    <p><code>node tools/generate-og-preview.mjs</code> - Preview local build (deploy/)</p>
    <p><code>node tools/generate-og-preview.mjs --local</code> - Preview dev server (localhost:4321)</p>
    <p><code>node tools/generate-og-preview.mjs --prod</code> - Preview production (gastownhall.ai)</p>
  </div>
</body>
</html>
`;
...
// Write the file
const outputPath = join(__dirname, 'og-preview.html');
writeFileSync(outputPath, html, 'utf-8');

console.log('✅ Generated: tools/og-preview.html');
console.log();
console.log('To view:');
console.log('  open tools/og-preview.html');
console.log();
